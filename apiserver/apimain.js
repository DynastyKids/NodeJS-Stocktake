...
const uri = encodeURI(
	credentials.mongodb_protocol +
	"://" +
	credentials.mongodb_username +
	":" +
	credentials.mongodb_password +
	"@" +
	credentials.mongodb_server +
...
async function insertOneToLog(insertData) {
	const inserterClient = new MongoClient(uri, {
		serverApi: {
			version: ServerApiVersion.v1,
			strict: true,
			deprecationErrors: true,
	...
	try {
		...
		const session = inserterClient.db(credentials.mongodb_db).collection("pollinglog");
