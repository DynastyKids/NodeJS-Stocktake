...
document.querySelector("#table_submit").addEventListener("click", function (ev) {
    ...
    for (let i = 0; i < productLists.length; i++) {
        ...
        for (let copy = 0; copy < rowInput[4].value; copy++) { // Looping when require multiple copies
            ...
            doc.text(qrCodeInfos.quantity + "  " + qrCodeInfos.unit, 15, doc.internal.pageSize.getHeight() - 190, {lineHeightFactor: 0.8});
            ...
            doc.addImage(qrCodeGenerateV3(qrCodeInfos.purchaseNo, qrCodeInfos.productCode, qrCodeInfos.productName, 1,
                    qrCodeInfos.quantity, qrCodeInfos.unit, qrCodeInfos.bestbefore, qrCodeInfos.labelId)
            ...
            doc.text(qrCodeInfos.labelId.slice(-7), doc.internal.pageSize.getWidth()-20, 18,
